...
public class CallCenterEi extends CallCenterDisc {
   ...
   private void computeCICV (DoubleArrayList A, DoubleArrayList X, double beta, double varMC) {
	...
	private double simulateWithoutStratification(int numRuns, int numPilotRuns) {
		...
		// print the properties of B distribution
   ...
   private void simulateWithStratificationCRN (int numRuns, int numPilotRuns, double varMC) {
	      println ("--------------------------------------");
	      println ("     STRATIFICATION, INDIRECT AND CRN     ");
	      ...
	      // PILOT RUNS
	      System.out.println("Running stratification pilot runs");
	      for (int t=0; t<k; t++) {
	         // Pilot runs with B = b[t].
	         statArrivals.init();     statWaits.init();
	         statGoodQoS.init();  statAbandon.init(); 
	...
	public static void main(String[] args) throws IOException {
		...
		CallCenterEi cc = new CallCenterEi("CallCenter.dat", b, q, gammaGenBeta);
		/*
		for(int i =0 ; i< 100; i++)
		...
		*/
		...
		double varMC = cc.simulateWithoutStratification(10000, 1000);
		
		System.out.println(varMC);
		/*
