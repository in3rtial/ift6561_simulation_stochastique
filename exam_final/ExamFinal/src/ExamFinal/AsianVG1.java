...
import umontreal.iro.lecuyer.stochprocess.GammaProcessSymmetricalBridge;
import umontreal.iro.lecuyer.stochprocess.VarianceGammaProcessDiff;

public class AsianVG1{
	   final double K;
	...
	public AsianVG1(double r, double sigma, double theta, double nu, double K,
			double s0, double T) {
		 this.K = K;
	      this.s0 = s0;
	...
	public void simulate(int n, double theta2, RandomStream prng, Tally stats, boolean jump) {
		...
		double p_lc = Math.pow(p_lambda, p_alpha) / Math.pow(p_lambda + theta2, p_alpha);
		double n_lc = Math.pow(n_lambda, n_alpha) / Math.pow(n_lambda + theta2, n_alpha);
		
		//System.out.println(p_lc);
		//System.out.println(n_lc);
		
		// simulate the system
		for (int i = 0; i < n; i++)
		{
			// the twisting is applied by adding or subtracting
			// to the scale parameter of the distributions
			double x_p = GammaGen.nextDouble(prng, p_alpha, p_lambda-theta2);
			...
			double L = likelihood_p * likelihood_n;
	...
	public static void main(String[] args) {
		...
		AsianVG1 process = new AsianVG1(r, sigma, theta, nu, K, s0, T);
		
		int n = 1000000;
		Tally stats = new Tally();
		process.simulate(n, 9, prng, stats, true);
